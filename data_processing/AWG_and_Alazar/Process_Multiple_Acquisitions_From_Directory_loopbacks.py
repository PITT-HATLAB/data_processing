...
# filepath = r'Z:/Data/C1/C1_Hakan/Gain_pt_0.103mA/signal_power_sweeps/3_mid_powers/2021-07-06/2021-07-06_0052_Amp_1__LO_freq_6153298714.0_Hz_Sig_Volt_0.5_V_Phase_0.0_rad_/2021-07-06_0052_Amp_1__LO_freq_6153298714.0_Hz_Sig_Volt_0.5_V_Phase_0.0_rad_.ddh5'

filepath = r'Z:/Data/C1/C1_Hakan/Gain_pt_0.102mA/loopbacks/2021-07-12/2021-07-12_0003_Amp_0__Phase_4.189_rad_/2021-07-12_0003_Amp_0__Phase_4.189_rad_.ddh5'
...
data_fidelity, fit_fidelity = PU.get_fidelity_from_filepath(filepath, plot = True, hist_scale = 0.005, records_per_pulsetype = 3840*2)
...
data = dd.DataDict(
        ...
        sigma_x_even = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        sigma_y_even = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        voltage_even = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        sigma_x_odd = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        sigma_y_odd = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        voltage_odd = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        sep_voltage = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV'),
        
        sep_over_sigma = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'mV/mV'),
        even_power_gain = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'dB'),
        odd_power_gain = dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'dB'),
        avg_power_gain =  dict(axes=['detuning', 'pump_pwr', 'phase'], unit = 'dB'),
        
        histogram_fidelity =  dict(axes=['detuning', 'pump_pwr', 'phase']), 
        histogram_infidelity = dict(axes=['detuning', 'pump_pwr', 'phase']),
        
        fit_fidelity =  dict(axes=['detuning', 'pump_pwr', 'phase']), 
        fit_infidelity = dict(axes=['detuning', 'pump_pwr', 'phase'])
        )
...
with dds.DDH5Writer(savedir, data, name=savename) as writer:
    for i in range(len(amp_on_filepaths)):
        ...
        try: 
            ...
            even_fit = PU.fit_2D_Gaussian(f'det_{det}_pwr_{pwr}_phase_{phase}_even', bins_even, h_even, 
                                                 guessParam[0], 
                                                 max_fev = 1000, 
                                                 contour_line = 2)
            
            odd_fit = PU.fit_2D_Gaussian(f'det_{det}_pwr_{pwr}_phase_{phase}_odd', bins_odd, h_odd, 
                                                guessParam[1],
                                                max_fev = 1000, 
                                                contour_line = 2)
