...
def getData(filename, method = 1):
    ...
    if(file_ext == "csv"):
        ...
        try:
            ...
            mag = np.array(f["S11"].value[0])
            phase = np.array(f["S11"].value[1])*np.pi/180.0
            lin = 10**(mag/20.0)
...
#method 2
def reflection(freq, real, imag, f_0_Guess = None, Q_int_Guess = None, Q_ext_Guess = None):
    '''
...
#       Q_ext=10**3 * Qtot
    ...
    fit_params = lmf.Parameters()
    if f_0_Guess == None: 
        #guess the middle of the trace
        f_0 = freq[np.floor(np.size(freq)/2)]
    else: 
        f_0 = f_0_Guess
    if Q_int_Guess == None: 
        Q_int = 1.9e3
    ...
    fit_params.add('A', value = 7.87-1, min=0, vary=True)
#    fit_params.add('B', value = np.pi - out['offset'],  vary= True)
    fit_params.add('B', value = -06,  vary=True)
#    fit_params.add('E', value = out['slope'], vary = True)
    fit_params.add('E', value = -6.0e-9, vary = True) 
...
if __name__ == '__main__':
    directory =  r"C:\Users\Hatlab_3\Desktop\20200723_RT_cav\\"
    filename = "1_unpolished_cav"
    (freq, real, imag) = getData(directory + filename)
    fit(freq, real, imag, method = 2, plot =True)