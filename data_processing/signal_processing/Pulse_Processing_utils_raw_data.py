...
def Process_One_Acquisition_3_state(filepath: str):
    '''
    Parameters
    ----------
    filepath : TYPE str
        filepath leading to the data 

    Returns
    -------
    The raw data from an acquisition, so that all the processing can be done on purpose by hand
    in the vort v(t) for the signal or reference
    '''
    dd = all_datadicts_from_hdf5(filepath)['data']
    timeUnit = dd['time']['unit']
    numTimePoints = np.size(np.unique(dd['time']['values']))
    numRecords = np.size(dd['time']['values'])//numTimePoints
    
    print("# of time points: ", numTimePoints, "\n# of Records: ", numRecords)
    
    # print(np.size(np.unique(dd['time']['values'])))
    timeVals = dd['time']['values'].reshape((numRecords, numTimePoints))
    
    recUnit = dd['record_num']['unit']
    recNum = dd['record_num']['values'].reshape((numRecords, numTimePoints))

    vG = dd['vG']['values'].reshape((numRecords, numTimePoints))
    vE = dd['vE']['values'].reshape((numRecords, numTimePoints))
    vF = dd['vF']['values'].reshape((numRecords, numTimePoints))
    
    rG = dd['rG']['values'].reshape((numRecords, numTimePoints))
    rE = dd['rE']['values'].reshape((numRecords, numTimePoints))
    rF = dd['rF']['values'].reshape((numRecords, numTimePoints))

    return timeVals, np.array([vG, vE, vF]), np.array([rG, rE, rF])
#     #averages
