...
class fit_fluxsweep():
    def __init__(self, Flux_filepath,
        ...
        if self.phaseUnit == 'rad':
            self.undriven_vna_phase = uvphDict.data_vals(phaseName)
        elif self.phaseUnit == 'deg':
            self.undriven_vna_phase = uvphDict.data_vals(phaseName)/360*2*np.pi
        else:
            raise Exception("The phase unit is not correct (either 'deg' or 'rad' required) ")
...
def reflectionFuncDbl(freq, Qext, Qint, f0, magBack, phaseCorrect):
    omega0 = f0
    delta = freq - omega0
...
class fit_doubleRes_fluxsweep():
    def __init__(self, Flux_filepath, save_filepath, name, currentName='current', phaseName='phase', powerName='power',
                 freqName='frequency'):
        # setup files
        self.name = name
        self.datadict = dd.DataDict(
    ...
    def initial_fit(self, f0Guess, QextGuess=50, QintGuess=300, magBackGuess=0.0001, bounds=None, smooth=False,
                    smooth_win=11, phaseOffGuess=0, debug=False, adaptive_window=False, adapt_win_size=300e6,
        ...
        popt, pconv = fit(init_vna_freqs, real, imag, init_pow_trace, init_phase_trace, Qguess=(QextGuess, QintGuess),
                          f0Guess=f0Guess, real_only=0, bounds=bounds, magBackGuess=magBackGuess,
                          phaseGuess=phaseOffGuess)
        ...
        for i in range(2):
            print(f'{fitting_params[i]}: {np.round(popt[i], 2)} +- {np.round(np.sqrt(pconv[i, i]), 3)}')
        Qtot = popt[0] * popt[1] / (popt[0] + popt[1])
        print('Q_tot: ', round(Qtot), '\nT1 (s):', round(Qtot / popt[2]),
              f"Kappa: {round(popt[2] / 2 / np.pi / Qtot)}", )
    ...
    def semiauto_fit(self, bias_currents, vna_freqs, vna_mags, vna_phases, popt, debug=False, savedata=False,
                     smooth=False, smooth_win=11, adaptive_window=False, adapt_win_size=300e6, fourier_filter=False,
                     fourier_cutoff=40, pconv_tol=2):
        res_freqs = np.zeros(np.size(np.unique(bias_currents)))
        Qints = np.zeros(np.size(np.unique(bias_currents)))
        Qexts = np.zeros(np.size(np.unique(bias_currents)))
        magBacks = np.zeros(np.size(np.unique(bias_currents)))
        ...
        for i, current in enumerate(np.unique(bias_currents)):
            first_condn = bias_currents == current
            [first_trace_freqs, first_trace_phase, first_trace_mag] = [vna_freqs[first_condn], vna_phases[first_condn],
                                                                       10 ** (vna_mags[first_condn] / 20)]
            ...
            if adaptive_window:
                if i == 0 or i == 1:
                    ...
                    print("freq window guess: ", res_freqs[i - 1:i])
                    filt1 = first_trace_freqs < np.average(
                        res_freqs[i - 1:i]) * 2 * np.pi + adapt_win_size * 2 * np.pi / 2
                    filt2 = first_trace_freqs > np.average(
                        res_freqs[i - 1:i]) * 2 * np.pi - adapt_win_size * 2 * np.pi / 2
                    filt = filt1 * filt2
            ...
            if debug:
                print('before fit: ', '\ninit_fo: ', init_f0, '\nfmin: ', np.min(first_trace_freqs), '\nfmax: ',
                      np.max(first_trace_freqs))
            popt, pconv = fit(first_trace_freqs[filt], real[filt], imag[filt], first_trace_mag[filt],
                              first_trace_phase[filt], Qguess=(QextGuess, QintGuess), f0Guess=f0Guess, real_only=0,
                              bounds=bounds, magBackGuess=magBackGuess)

            # catch a sudden change in convergence and try again until it's back in range:
            if i > 2:
                pconv_diff_ratio = (np.array(pconv[0, 0], pconv[1, 1]) - np.array(prev_pconv[0, 0],
                                                                                  prev_pconv[1, 1])) / np.array(
                    prev_pconv[0, 0], prev_pconv[1, 1])
                if debug:
                    print(f"Pconv ratio: {pconv_diff_ratio}")
                j = 0
                alt_array = np.array(
                    [1e6, -1e6, 5e6, -5e6, 10e6, -10e6, 15e6, -15e6, 20e6, -20e6, 30e6, -30e6]) * 2 * np.pi
                while np.any(np.abs(pconv_diff_ratio) > pconv_tol):
                    if j > 11:
                        raise Exception("No good fit at this point")
                    print(
                        f"sudden change in Q detected (pconv_diff_ratio: {pconv_diff_ratio}), trying resonant guess + {alt_array[j] / (2 * np.pi)}")
                    # try above
                    if debug:
                        if j % 2 == 0:
                            print("trying above")
                            ...
                            print("trying_below")
                    popt, pconv = fit(first_trace_freqs[filt], real[filt], imag[filt], first_trace_mag,
                                      first_trace_phase, Qguess=(QextGuess, QintGuess), f0Guess=f0Guess + alt_array[j],
                                      real_only=0, bounds=bounds, magBackGuess=magBackGuess)
            ...
            if debug:
                import time
                plotRes(first_trace_freqs[filt], real[filt], imag[filt], first_trace_mag[filt], first_trace_phase[filt],
                        popt)
                time.sleep(1)
