...
CSV = r'E:\Ryan_Files\OneDrive - University of Pittsburgh\slides_figures\NISTAmp_2022\raw_plots/Sivak_Fig_vals_no_KTWPA.csv'
df = pd.read_csv(CSV, error_bad_lines = 'warn')
fig, ax = pplt.subplots(refwidth = '3in')
...
for key, device in df.T.items():
    if key == list(df.T.items())[-1][0]: 
        ...
        color = 'black'
    ax.scatter(device['Pump Power (dBm)'], device['Output Saturation Power (dBm)'], color = device['Color'], markersize = markersize, label = str(device['Device'])+', '+str(device['Author'])+'('+str(device['Year'])+')')
    #plot the pla device
    # ax.scatter(-19.8, -52, label = 'KIPA: PHYSICAL REVIEW APPLIED 17, 034064 (2022)')
xvals = np.linspace(-100, -10, 100)
ax.plot(xvals, line50(xvals), 'k-', label = '100% PAE')
ax.plot(xvals, line001(xvals), 'k--', label = '0.1% pump utilization')
