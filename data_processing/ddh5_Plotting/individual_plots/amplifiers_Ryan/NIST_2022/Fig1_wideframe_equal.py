...
gain_fps = [data_home_dir+r'/20dB_wide_2/2022-05-24_0001_0.16mA.ddh5', 
    data_home_dir+r'/18dB_wide/2022-05-24_0001_0.16mA.ddh5', 
            data_home_dir+r'/20dB_narrow/2022-05-24_0002_0.16mA.ddh5']

sat_fps = [data_home_dir+r'/20dB_wide_2/2022-05-24_0002_sat_amp_on.ddh5', 
           data_home_dir+r'/18dB_wide/2022-05-24_0002_sat_amp_on.ddh5', 
           data_home_dir+r'/20dB_narrow/2022-05-24_0005_sat_amp_on.ddh5']

sat_sb_fps = [data_home_dir+r'/20dB_wide_2/2022-05-24_0003_sat_amp_off.ddh5', 
              data_home_dir+r'/18dB_wide/2022-05-24_0003_sat_amp_off.ddh5',
              data_home_dir+r'/20dB_narrow/2022-05-24_0006_sat_amp_off.ddh5']

NVR_fps = [data_home_dir+r'/20dB_wide_2/2022-05-24_0005_NVR_amp_on.ddh5',
           data_home_dir+r'/18dB_wide/2022-05-24_0005_NVR_amp_on.ddh5', 
           data_home_dir+r'/20dB_narrow/2022-05-24_0003_NVR_amp_on.ddh5']

NVR_sb_fps = [data_home_dir+r'/20dB_wide_2/2022-05-24_0004_NVR_amp_off.ddh5',
              data_home_dir+r'/18dB_wide/2022-05-24_0006_NVR_amp_off.ddh5', 
...
fig, axs = pplt.subplots(ncols = 3, nrows = 1, sharex = False, sharey = False)
for ax in axs: 
...
gain_colors = [greencmap(0.5), newcmp(cnum[0]), newcmp(cnum[2])]
noise_colors = gain_colors

gain_labels = ['Pump frequency = ', ]
# 
...
for i, (gain_fp, sat_fp, sat_sb_fp, NVR_fp, NVR_sb_fp) in enumerate(zip(gain_fps, sat_fps, sat_sb_fps, NVR_fps, NVR_sb_fps)):
    ...
    pump_freq = np.round(2*np.average(freqs[ffilt])/1e9, 3)
    pump_freq_from_center = np.round(2*np.average(freqs[ffilt])/1e6-11252, 0)
    gain_ax.plot(((freqs[ffilt]-np.average(freqs[ffilt]))/1e6)[::skip], pows[ffilt][::skip], 's', label = '$f_p$ = '+str(pump_freq)+' GHz', color = gain_colors[i])
    gain_ax.plot(((freqs[ffilt]-np.average(freqs[ffilt]))/1e6)[::skip], pows[ffilt][::skip], 's-', color = gain_colors[i])
    
    print(f"\n\n\n2xcenter: {2*np.average(freqs[ffilt])/1e6}\n\n\n")
    #plot the NVR
    sb_fp = NVR_sb_fp
    fp = NVR_fp
...
fig.legend(loc = 'top', ncol = 3, fontsize = 0.1)
fig.save(r'C:\Users\Ryan\OneDrive - University of Pittsburgh\slides_figures\gain_sat_wideframe.svg')
sat_ax.grid()
gain_ax.grid()

#%% make a dummy plot of saturation using only one of the traces
sat_fp = sat_fps[1]
satfig, sat_ax = pplt.subplots()
satax.set_xlabel('Input Power')
satax.set_ylabel('Gain')
...
att = 85
sat_ax.plot((freqs-att)[::satskip], (pows-norm_pows)[::satskip]+0.1, 's-', color = 'k')
sat_ax.set_xlabel('Signal power (dBm)')
sat_ax.set_ylabel('Gain (dB)')
# sat_ax.legend()
sat_ax.set_ylim(18.5, 20.5)
sat_ax.hlines([19], np.min(freqs)-att, np.max(freqs)-att, linestyle = '--', color = 'k')
sat_ax.set_xticks([-120, -110, -100, -90, -80])
sat_ax.set_xticklabels(["", '', '', "$P_{-1dB}$", ''])
sat_ax.set_title("Gain Saturation")

satfig.save(r'C:\Users\Ryan\OneDrive - University of Pittsburgh\slides_figures\sat_example.svg')